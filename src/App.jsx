  ...
  if (stepKey === "essay") {
    ...
    if (stepLevel === 2) return {
      ...
      selfHelp: [
        ...
        "탈락한 회사에서 합격한 사람의 자소서가 있다면 비교 분석하세요. 본인 글과 어디가 다른지 보면 진짜 차이가 보입니다. 합격 자소서 비교 분석은 자소서 멘토링에서 함께 진행할 수도 있습니다.",
...
function analyze(ans) {
  ...
  if (levels.job === 0) {
    ...
    addAction(0, who === "new" ? { 
      ...
      text: "연구 경력을 R&D/일반직/공공기관 3트랙으로 매핑하세요",
      detail: "박사·석사 경력을 ①민간 R&D(대기업 연구소·바이오테크 등) ②기업 일반직(컨설팅·전략기획) ③공공기관(연구원·국책기관) 3가지 트랙으로 분류해 각 트랙의 채용공고를 5개씩 검토하세요. 학위가 가장 큰 자산인 트랙과 오히려 부담이 되는 트랙이 명확히 갈립니다."
    });
    docs.push({ 
      n: "채용공고 및 직무 분석 가이드북", 
    ...
    addAction(0, { text: "후보를 1~2개로 좁혀서 확정하세요", detail: "'나는 OO 직무에 지원할 것이다'를 한 문장으로 말할 수 있으면 됩니다. 결정을 1주일 이상 미루지 마세요 — 결정의 무게보다 미루는 비용이 더 큽니다." });
  }
  ...
  if (levels.job >= 1 && levels.jd === 0) {
    ...
    addAction(1, { text: "반복되는 키워드 3~5개를 뽑으세요", detail: "10개 공고에서 계속 나오는 단어가 이 직무의 '핵심 역량'입니다. 이 키워드가 자소서·이력서의 뼈대가 됩니다. 단순히 단어를 나열하지 말고, 각 키워드 옆에 본인이 그것을 보유한 증거(경험 1줄)를 적으세요." });
    docs.push({ 
      n: "채용공고 및 직무 분석 가이드북", 
      u: "https://www.latpeed.com/products/-3Wgm",
  ...
  if (levels.essay >= 1 && levels.resume >= 1 && levels.interview === 0) {
    ...
    docs.push({ 
      n: who === "new" ? "신입 면접 준비 가이드북" : "신입 면접 준비 가이드북", 
      u: "https://www.latpeed.com/products/H7UHo",
      w: who === "new" ? "신입 면접의 80%는 비슷한 20개 질문 안에 있습니다. 가이드북은 자기소개, 지원동기, 성격 장단점, 직무 이해도, 인성 질문 등 핵심 카테고리별 답변 프레임과 실제 합격자 답변 예시를 제공합니다. 자소서 기반 예상 질문 추출 워크북이 가장 실용적입니다." : "면접 답변 구조(STAR + 1)와 꼬리질문 대비법은 신입·경력 모두 동일하게 적용됩니다. 신입 가이드북에 포함된 답변 프레임을 기반으로 본인의 경력 깊이에 맞게 활용 가능합니다."
    });
    docs.push({ 
      n: "면접 멘토링", 
...
export default function App() {
  ...
  onMouseLeave={e => e.currentTarget.style.opacity = 1}>
        ...
        <div style={{textAlign:"center",padding:"20px 0 32px",display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={handleSaveResult} disabled={isSavingDocx}
            style={{display:"inline-flex",alignItems:"center",justifyContent:"center",gap:8,padding:"12px 32px",borderRadius:12,background:isSavingDocx ? COLORS.sub : COLORS.accent,color:COLORS.white,cursor:isSavingDocx?"wait":"pointer",fontSize: 16,fontWeight:600,border:"none",fontFamily:"inherit",opacity: isSavingDocx ? 0.7 : 1,transition:"opacity 150ms"}}>
            {isSavingDocx ? (
              <>

// [BUILD v55 20260520] essay=3 + 낮은 통과율 메시지 톤 재정립 — '운으로 통과'에서 '다른 지원자가 먼저 떨어진 결과'로 표현 변경, 타인 의존적 사고('내가 합격')에서 본인 통제권 회복('내가 통제할 수 있는 자소서 완성도')으로 전환. essay lv=3 가이드(situation/concerns/selfCheck/vision/selfHelp)와 essayWarning, stageNote, 첫 액션 detail까지 일관 적용
  ...
  if (stepKey === "jd") {
    if (stepLevel === 0) return {
      ...
      concerns: who === "grad" ? [
        ...
        "연구직(R&D) 트랙과 일반직 트랙을 어떻게 비교해야 할지 모르겠다",
      ...
      selfHelp: who === "grad" ? [
        ...
        "같은 직무라도 [연구직 공고]와 [일반직 공고]를 각 2~3개씩 비교해보세요. 자격요건 표현이 다르고, 학위자 우대 표현의 유무로 어느 트랙이 본인에게 유리한지 파악됩니다.",
  ...
  if (stepKey === "jd") {
    if (level === 0) return {
      ...
      guidance: who === "new" ? "직무명만 알고 실제 업무는 추상적인 상태입니다. 현직자 콘텐츠(유튜브·브런치·링크드인) 3개 + 채용공고 자격요건·주요업무를 비교하면서, '하루를 어떻게 보내는지·어떤 산출물을 만드는지'를 노트에 적으세요. 신입은 '직무가 뭘 하는지 자체'를 모르는 게 정상이라, 머리로 상상하지 말고 실제 자료를 모아야 합니다."
        ...
        : "연구 분야와 직무가 어떻게 연결되는지 약한 상태입니다. 본인 연구 주제를 5개 키워드로 압축한 뒤, 그 키워드로 채용 사이트에서 검색해 어떤 직무가 나오는지 매핑하세요. R&D 트랙·기업 일반직·공공기관은 평가 기준이 완전히 다릅니다."
    };
    if (level === 1) return {
...
export default function App() {
  ...
  const handleSaveResult = async () => {
    ...
    try {
      ...
      const _gradHint = (_who === 'grad') ? ' · 박사·R&D 직무 포함' : '';
